...
from model.layers.regress_boxes import shift
...
class Anchors(tf.keras.layers.Layer):
	...
	def call(self, inputs, **kwargs):
		...
		if tf.keras.backend.image_data_format() == 'channels_first':
			anchors = shift(image_shape[2:4], features_shape[2:4], self.stride, self.anchors)
		else:
			anchors = shift(image_shape[1:3], features_shape[1:3], self.stride, self.anchors)
